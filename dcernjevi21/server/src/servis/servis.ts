...
function pokreniKonfiguraciju() {
  ...
  server.post(
    "/servis/app/setPravo/:korime/:vrijednost",
    (zahtjev, odgovor) => {
      let korisnikDao = new KorisnikDAO();
      korisnikDao.oznaciPristupRest(
        zahtjev.params["korime"],
        Number(zahtjev.params["vrijednost"])
      );
      odgovor.end();
    }
  );
  ...
  server.post(
    "/servis/app/korisnici/2f/:korime/:vrijednost/:generirajTotp",
    async (zahtjev, odgovor) => {
      let korisnikDao = new KorisnikDAO();
      korisnikDao.azuriraj2fAutentifikaciju(
        zahtjev.params["korime"],
        Number(zahtjev.params["vrijednost"]),
        Number(zahtjev.params["generirajTotp"])
      );
      odgovor.send(
        JSON.stringify(
          await korisnikDao.dohvatiKorisnika(zahtjev.params["korime"])
        )
      );
    }
  );
  ...
  server.get(
    "/servis/app/korisnici/2f/totp/:username/:key",
    async (zahtjev, odgovor) => {
      let korisnikDao = new KorisnikDAO();

      let korisnik = (await korisnikDao.dohvatiKorisnika(
        zahtjev.params["username"]
      )) as KorisnikI;
  ...
  server.post(
    "/servis/app/checkRecaptcha",
    async (zahtjev: any, odgovor: any) => {
  ...
  server.use(
    "/",
    express.static(path.join(putanja, "..", "..", "angular", "browser"))
  );
...
function pripremiPutanjeAutentifikacija() {
  ...
  server.get(
    "/servis/app/dodavanje",
    htmlUpravitelj.dohvatOsoba.bind(htmlUpravitelj)
  );
...
function pripremiPutanjeResursTMDB() {
  ...
  server.get(
    "/servis/app/tmdb/images/:id",
    restTMDB.getPersonImages.bind(restTMDB)
  );
  ...
  server.delete(
    "/servis/app/osoba",
    restTMDB.deleteOsobaStranica.bind(restTMDB)
  );
  ...
  server.get(
    "/servis/app/osoba/:id/film",
    restTMDB.getOsobaFilm.bind(restTMDB)
  );
  server.post(
    "/servis/app/osoba/:id/film",
    restTMDB.postOsobaFilm.bind(restTMDB)
  );
  server.put(
    "/servis/app/osoba/:id/film",
    restTMDB.putOsobaFilm.bind(restTMDB)
  );
  server.delete(
    "/servis/app/osoba/:id/film",
    restTMDB.deleteOsobaFilm.bind(restTMDB)
  );
  ...
  server.delete(
    "/servis/app/film/brisanjeSvihNepovezanih",
    restTMDB.deleteFilmoveNepovezane.bind(restTMDB)
  );
}
function pripremiPutanjeResursKorisnika() {
  let restKorisnik = new RestKorisnik();
  server.get(
    "/servis/app/korisnici",
  ...
  server.put(
    "/servis/app/korisnici",
    restKorisnik.putKorisnici.bind(restKorisnik)
  );
  server.delete(
    "/servis/app/korisnici",
  ...
  server.get(
    "/servis/app/korisnici/:korime",
    restKorisnik.getKorisnik.bind(restKorisnik)
  );
  server.post(
    "/servis/app/korisnici/:korime/prijava",
